...
% Manuscript fragment: regression-results table body (bold rows: p < 0.005)
...
for n = 1:height(stats_table)
    sl = sprintf('%.3f',stats_table.Line_Slope(n));
    pv = sprintf('\\num{%.3e}',stats_table.Line_Slope_pValue(n));
    r2 = sprintf('%.3f',stats_table.R2(n));
    if stats_table.Line_Slope_pValue(n) < 0.005
        fprintf(fid,'\\textbf{%s} & \\textbf{%s} & \\textbf{%s} & \\textbf{%s} \\\\ \\hline\n',row_names{n},sl,pv,r2);
    else
