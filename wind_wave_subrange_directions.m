...
wave_age_high = 90;
wave_age(wave_age>wave_age_high) = NaN;
m.breakers.wave_age(m.breakers.wave_age>wave_age_high) = NaN;
m.current.wave_age(m.current.wave_age>wave_age_high) = NaN;
...
rng(0)   % reproducible wild cluster bootstrap for the tabulated slope CIs
...
for n = 1:6
    ...
    plot([-180 180],[-180 180],':','Color',0.5*[1 1 1],'linewidth',3)
    ...
    c = c(inds_keep);
    blk = blk(inds_keep);
    xplot = linspace(-fit_lim+dD/2,fit_lim-dD/2,100);

    inds_fit = x>-fit_lim+dD/2 & x<fit_lim-dD/2;
    ...
    % Slope CI from a wild cluster bootstrap over acquisition blocks, so the
    ...
    scatter(x,y,0.6*msize^2,wc.transform(c),'filled')
...
% The complete regression-results tabular
if ~exist('figs/tex','dir'); mkdir('figs/tex'); end
fid = fopen('figs/tex/table_regression_results.tex','w');
fprintf(fid,'%% Auto-generated by wind_wave_subrange_directions.m; do not edit by hand\n');
fprintf(fid,'\\begin{tabular}{|l|l|l|l|}\n');
fprintf(fid,'\\hline\n');
fprintf(fid,'Subrange & Line Slope & Slope 95\\%% CI & $R^2$ \\\\ \\hline\n');
for n = 1:height(stats_table)
    lo = stats_table.Slope_CI_Lower(n);
    hi = stats_table.Slope_CI_Upper(n);
    cells = {row_names{n}, ...
             sprintf('%.3f',stats_table.Line_Slope(n)), ...
             sprintf('[%.3f, %.3f]',lo,hi), ...
             sprintf('%.3f',stats_table.R2(n))};
    if lo*hi > 0
        cells = cellfun(@(s) sprintf('\\textbf{%s}',s),cells,'UniformOutput',false);
    end
